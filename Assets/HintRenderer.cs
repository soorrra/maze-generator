...
public class HintRenderer : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        componentLineRenderer.enabled = false; // Скрываем подсказку при старте
    }

    public void ToggleHint()
    {
        isHintVisible = !isHintVisible;
        componentLineRenderer.enabled = isHintVisible;

        if (isHintVisible && MazeSpawner.maze != null)
        {
            DrawPath();
        }
    }
    ...
    public void DrawPath()
    {
        ...
        Debug.Log($"Drawing path from finish at {maze.finishPosition}");
    }
}