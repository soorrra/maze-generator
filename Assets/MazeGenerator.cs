...
public class MazeGenerator
{
    ...
    private void RemoveWallsWithBacktracker(MazeGeneratorCell[,] maze)
    {
        ...
        do
        {
            ...
            if (unvisitedNeighbours.Count > 0)
            {
                ...
                AddDeadEnds(current, stack, maze);
            }
            else
            {
                current = stack.Pop();
    ...
    private void AddDeadEnds(MazeGeneratorCell current, Stack<MazeGeneratorCell> stack, MazeGeneratorCell[,] maze)
    {
        float deadEndChance = Difficulty switch
        {
            DifficultyLevel.Easy => 0.1f,
            DifficultyLevel.Medium => 0.3f,
            DifficultyLevel.Hard => 0.5f,
            _ => 0.3f
        };

        if (UnityEngine.Random.value < deadEndChance)
        {
            int x = current.X;
            int y = current.Y;

            if (x > 0 && !maze[x - 1, y].Visited && UnityEngine.Random.value > 0.5f)
                current.WallLeft = true;
            if (y > 0 && !maze[x, y - 1].Visited && UnityEngine.Random.value > 0.5f)
                current.WallBottom = true;
            if (x < Width - 2 && !maze[x + 1, y].Visited && UnityEngine.Random.value > 0.5f)
                maze[x + 1, y].WallLeft = true;
            if (y < Height - 2 && !maze[x, y + 1].Visited && UnityEngine.Random.value > 0.5f)
                maze[x, y + 1].WallBottom = true;
        }
    }
